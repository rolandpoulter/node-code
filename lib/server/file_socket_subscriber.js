...
FileSocketSubscriber.prototype.onOpenFile = function (socket, project_name, relative_file_path) {
	...
	console.log('open file', project_name, relative_file_path);

	var that = this;// TODO: use .bind

	this.Project.get(project_name).getFile(relative_file_path).open(function (file_data) {
		that.file_publisher.emitFileOpen(socket, project_name, relative_file_path, file_data);
	});
